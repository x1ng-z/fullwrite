...
public class test {
    ...
    public static int CRC16_CalateByte(int /*U8*/ CheckByte,int /* U16*/ LastCRC)
    {
        return (( (LastCRC&0xffff) << 8 ) ^ (0xffff&CRCtable [ ( (LastCRC&0xffff) >>8 ) ^ (CheckByte&0xff) ]))&0xffff;
    }


    static int CRC16_CalculateBuf(int[] ptr, int len)
    {
        int crc_result = 0x0000;
        int index=0;
        while(index<len)
        {
            crc_result = CRC16_CalateByte(ptr[index], crc_result );
            index++;
        }
    ...
    public static void main(String[] args) {
        int[] a=new int[]{0x11, 0x12, 0x00, 0x04, 0x00, 0x00, 0x00, 0x8A};
        int result=CRC16_CalculateBuf(a,8);
        System.out.println(String.format("crc1=%x,crc2=%x\n",(result>>8&0xff),(result>>0&0xff)));
        /*CRC校验表，切勿更改*/
